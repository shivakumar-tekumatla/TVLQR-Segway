function [problem,guess] = Segway
%TwoLinkRobotArm - Time optimal control of a two-link robot arm
%
% The problem was adapted from Example 2, Section 12.4.2 of
% R. Luus. Iterative Dynamic Programming. Chapman & Hall/CRC Monographs and Surveys in Pure and Applied Mathematics, 2000.
%
% Syntax:  [problem,guess] = TwoLinkRobotArm
%
% Outputs:
...
InternalDynamics=@Segway_dynamics_internal;
SimDynamics=@Segway_dynamics_sim;
...
% Settings file
problem.settings=@settings_Segway;
...
problem.states.x0=[0.5,pi/3,pi/6,0,0,0];

% Initial conditions for system. Bounds if x0 is free s.t. x0l=< x0 <=x0u
problem.states.x0l=[-1,-pi,-pi/2,0,0,0]; %[0.5,pi/3,pi/6,0,0,0];%
problem.states.x0u=[1,pi,pi/2,0,0,0]; %[0.5,pi/6,pi/3,0,0,0];%

% State bounds. xl=< x <=xu
problem.states.xl=[-1,-2*pi,-2*pi,-5,-5,-5]; %[-inf -inf -inf -inf -inf -inf];%[-1,-2*pi,-2*pi,-5,-5,-5];
problem.states.xu=[1,2*pi,2*pi,5,5,5];%[inf inf inf inf inf inf ];%[1,2*pi,2*pi,5,5,5]

% State error bounds
problem.states.xErrorTol_local=[0.01 deg2rad(0.001) deg2rad(0.001) 0.001 deg2rad(0.0005) deg2rad(0.0005)];
problem.states.xErrorTol_integral=[0.01 deg2rad(0.001) deg2rad(0.001) 0.001 deg2rad(0.0005) deg2rad(0.0005)];

% State constraint error bounds
problem.states.xConstraintTol=[0.01 deg2rad(0.001) deg2rad(0.001) 0.001 deg2rad(0.0005) deg2rad(0.0005)];

% Terminal state bounds. xfl=< xf <=xfu
problem.states.xfl=[1e-3 1e-3 1e-3 1e-3 1e-3 1e-3]; 
problem.states.xfu=[1e-3 1e-3 1e-3 1e-3 1e-3 1e-3]; 

% Guess the state trajectories with [x0 xf]
guess.states(:,1)=[0.5  0];
guess.states(:,2)=[pi/3 0];
guess.states(:,3)=[pi/6 0];
guess.states(:,4)=[0 0];
guess.states(:,5)=[0 0];
guess.states(:,6)=[0 0];
...
% Input bounds
problem.inputs.ul=[-5 -5];
problem.inputs.uu=[5 5];

problem.inputs.u0l=[-5 -5];
problem.inputs.u0u=[5 5];
